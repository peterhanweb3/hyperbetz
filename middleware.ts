...
export async function middleware(request: NextRequest) {
	...
	if (pathname === "/providers" && searchParams.has("category")) {
		...
		if (category) {
			...
			return NextResponse.redirect(
				new URL(`/providers/${seoCategory}`, request.url)
			);
		}
	}
	...
	if (pathname === "/games" && searchParams.has("provider_name")) {
		...
		if (provider) {
			...
			if (category) {
				const seoCategory = category.toLowerCase().replace(/\s+/g, "-");
				return NextResponse.redirect(
					new URL(`/games/${seoProvider}/${seoCategory}`, request.url)
				);
			} else {
				return NextResponse.redirect(
					new URL(`/games/${seoProvider}`, request.url)
				);
			}
		}
	}
